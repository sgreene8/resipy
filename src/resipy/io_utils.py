...
def calc_ray_quo(r_dict, prev_vec, curr_vec, iter_num, eps):
    ...
    prev_vec : (SparseVector)
        Iterate from the previous step
    curr_vec : (SparseVector)
        Iterate from the current step
    iter_num : (unsigned int)
        index of the current iteration in the trajectory
    eps : (float)
        Imaginary time step for the simulation
    """
    buf_len = r_dict['buf_len']
    shift_int = r_dict['shift_int']
    res_idx = (iter_num % buf_len) / shift_int

    self_overlap = numpy.linalg.norm(prev_vec.values)**2
    r_dict['ray_den'][1][res_idx] = self_overlap

    num_overlap = prev_vec.dot(curr_vec)
    r_dict['ray_num'][1][res_idx] = (self_overlap - num_overlap) / eps

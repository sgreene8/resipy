...
def virt_symm(unsigned char[:, :] occ_orbs, unsigned char[:] orb_symm,
    ...
    for det_idx in range(num_dets):
        _count_symm_virt(virt_counts[det_idx], occ_orbs[det_idx],
                         lookup_tabl, orb_symm)
    return virt_counts
...
def sing_allow(unsigned int[:, :, :] virt_counts, unsigned char[:, :] occ_orbs,
    ...
    for det_idx in range(num_dets):
        ...
        for elec_idx in range(num_elec):
            ...
            symm_allowed = virt_counts[det_idx, elec_idx / (num_elec / 2),
                                       occ_symm]
...
def symm_pair_wt(unsigned int[:, :, :] virt_counts, unsigned char[:, :] occ_orbs,
    ...
    for sampl_idx in range(n_samples):
        ...
        occ = _tri_to_occ_pair(occ_orbs[curr_det], opair_tri[sampl_idx])
        ...
        if occ_same:
            for symm_idx in range(num_symm_pair):
                n_symm1 = virt_counts[curr_det, occ.spin1, xor_idx[xor_row_idx][symm_idx]]
                if n_symm1 > 1:
            ...
            for symm_idx in range(num_symm_pair):
                n_symm1 = virt_counts[curr_det, occ.spin1, xor_idx[xor_row_idx][symm_idx]]
                n_symm2 = virt_counts[curr_det, occ.spin2, sym_prod ^ xor_idx[xor_row_idx][symm_idx]]
...
def id_doub_virt(long long[:] dets, unsigned char[:, :] lookup_tabl,
    ...
    for det_idx in range(num_dets):
        ...
        if occ_same:
            ...
            xor_row_idx = virt_counts[det_idx, spin1, symm1]
...
cdef orb_pair _choose_occ_pair(unsigned char[:] occ_orbs, mt_struct *mt_ptr
    ...
    cdef unsigned int rand_pair = _choose_uint(mt_ptr, num_elec * (num_elec - 1)
                                               / 2)
    return _tri_to_occ_pair(occ_orbs, rand_pair)


cdef orb_pair _tri_to_occ_pair(unsigned char[:] occ_orbs, unsigned int tri_idx) nogil:
